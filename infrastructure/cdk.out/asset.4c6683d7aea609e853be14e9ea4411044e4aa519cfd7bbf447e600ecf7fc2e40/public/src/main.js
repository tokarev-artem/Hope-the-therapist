...
async function initAudio() {
    try {
        ...
        // Ensure start button is disabled during initialization
        startButton.disabled = true;

        // Check for browser compatibility and secure context
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            let errorMessage = 'Your browser does not support audio recording. ';
            
            // Check if it's a secure context issue
            if (location.protocol !== 'https:' && location.hostname !== 'localhost') {
                errorMessage += 'Please access this site via HTTPS or use localhost for development.';
            } else {
        ...
        // Verify audio context and stream are properly initialized
        if (!audioContext || !audioStream) {
            throw new Error('Audio initialization failed - missing audioContext or audioStream');
        }
        ...
        statusElement.textContent = "Error: " + error.message + " - Please refresh and allow microphone access.";
        statusElement.className = "error";
        
        // Ensure start button remains disabled on error
        startButton.disabled = true;
...
async function initializeSession() {
    ...
    try {
        // Send events in sequence with proper delays for Bedrock processing
        console.log('🎯 Step 1: Sending promptStart');
        socket.emit('promptStart');
        
        // Wait a bit for prompt start to be processed
        await new Promise(resolve => setTimeout(resolve, 100));

        // Send initial system prompt (required by Bedrock)
        const contextualPrompt = getContextualizedSystemPrompt();
        console.log('🎯 Step 2: Sending system prompt to Sonic:', contextualPrompt.substring(0, 500) + '...');
        socket.emit('systemPrompt', contextualPrompt);
        
        // Wait for system prompt to be processed
        await new Promise(resolve => setTimeout(resolve, 200));

        console.log('🎯 Step 3: Sending audioStart');
        socket.emit('audioStart');
        
        // Wait for audio start to be processed
        await new Promise(resolve => setTimeout(resolve, 100));
...
async function startStreaming() {
    ...
    try {
        ...
        if (!audioContext || !audioStream) {
            console.error('Audio not initialized. audioContext:', !!audioContext, 'audioStream:', !!audioStream);
            statusElement.textContent = "Audio not initialized. Please refresh and allow microphone access.";
            statusElement.className = "error";
            return;
        }
